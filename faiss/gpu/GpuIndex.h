...
class GpuIndex : public faiss::Index {
   public:
    ...
    void add_with_ids(idx_t n, const void* x,  NumericType numeric_type, const idx_t* xids) override;
   ...
   protected:
    ...
    void copyFrom(const faiss::Index* index, NumericType numeric_type) {
        if (numeric_type == NumericType::Float32) {
                ...
                throw std::runtime_error("GpuIndex::copyFrom: unsupported numeric type");
        }
    }
    ...
    virtual void addImpl_(idx_t n, const void* x, NumericType numeric_type, const idx_t* ids) {
        if (numeric_type == NumericType::Float32) {
...
const SearchParameters* params) const {
        if (numeric_type == NumericType::Float32) {
                searchImpl_(n, static_cast<const float*>(x), k, distances, labels, params);
                } else {
   ...
   private:
    ...
    void addPaged_(idx_t n, const void* x, NumericType numeric_type,  const idx_t* ids);

    /// Calls addImpl_ for a single page of GPU-resident data
    void addPage_(idx_t n, const float* x, const idx_t* ids);
    void addPage_(idx_t n, const void* x, NumericType numeric_type, const idx_t* ids);
