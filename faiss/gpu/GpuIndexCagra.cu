...
void GpuIndexCagra::train(idx_t n, const void* x, NumericType x_type) {

    if (x_type == NumericType::Float32) {
        train(n, static_cast<const float*>(x));
        return;
    } else if (x_type == NumericType::Float16) {
        ...
        index_->train(n, static_cast<const half*>(x));

        this->is_trained = true;
        this->ntotal = n;
    } else {
        throw std::runtime_error("GpuIndexCagra::train: unsupported numeric type");
    }
